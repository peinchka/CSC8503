...
NavigationGrid::NavigationGrid(const std::string&filename) : NavigationGrid() {
	...
	for (int y = 0; y < gridHeight; ++y) {
		for (int x = 0; x < gridWidth; ++x) {
			...
			for (int i = 0; i < 4; ++i) {
				if (n.connected[i]) {
					if ((n.connected[i]->type == '.') || (n.connected[i]->type == 'b') || (n.connected[i]->type == 'r')) {
						n.costs[i]		= 1;
					}
					else {
						n.connected[i] = nullptr; //Actually an obstacle - disconnect!
					}
				}
			}
		}	
...
bool NavigationGrid::FindPath(const Vector3& from, const Vector3& to, NavigationPath& outPath) {
	...
	int fromX = (from.x / nodeSize + 0.5);
	int fromZ = (from.z / nodeSize + 0.5);
	
	int toX = (to.x / nodeSize + 0.5);
	int toZ = (to.z / nodeSize + 0.5);
