...
bool CollisionDetection::BallGoalIntersection(GameObject* ball, GameObject* goal) {

	const CollisionVolume* volumeA = ball->GetBoundingVolume();
	const CollisionVolume* volumeB = goal->GetBoundingVolume();

	const Transform& TransformA = ball->GetConstTransform();
	const Transform& TransformB = goal->GetConstTransform();

	Vector3 boxSize = ((AABBVolume&)*volumeB).GetHalfDimensions();
	...
	if (distance < ((SphereVolume&)*volumeA).GetRadius()) {// yes , we 're colliding !
		return true;
	}
	return false;
}

bool CollisionDetection::BallRobotIntersection(GameObject* ball, GameObject* robot) {

	const CollisionVolume* volumeA = ball->GetBoundingVolume();
	const CollisionVolume* volumeB = robot->GetBoundingVolume();

	const Transform& TransformA = ball->GetConstTransform();
	const Transform& TransformB = robot->GetConstTransform();
